...
function clearItem() {
  ...
  localStorage.removeItem('live2d-enabled');
...
// 更换背景(原来Leonus的代码)
const LIVE2D_STORAGE_KEY = "live2d-enabled";
const LIVE2D_SCRIPT_URL = "/live2dw/lib/L2Dwidget.min.js";
const LIVE2D_OPTIONS = {
  model: { jsonPath: "/live2dw/assets/koharu.model.json" },
  display: { position: "left", width: 150, height: 300 },
  mobile: { show: true },
};

function isLive2dEnabled() {
  return localStorage.getItem(LIVE2D_STORAGE_KEY) !== "false";
}

function loadLive2d() {
  if (!isLive2dEnabled() || document.getElementById("live2d-widget")) return;

  const script = document.createElement("script");
  script.src = LIVE2D_SCRIPT_URL;
  script.onload = function () {
    if (isLive2dEnabled() && window.L2Dwidget) {
      window.L2Dwidget.init(LIVE2D_OPTIONS);
    }
  };
...
function setLive2d() {
  localStorage.setItem(LIVE2D_STORAGE_KEY, document.getElementById("live2dSet").checked ? "true" : "false");
  window.location.reload();
}
...
<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 看板娘 (刷新生效) </div><input type="checkbox" id="live2dSet" onclick="setLive2d()">
</div>
...
`;
  ...
  document.getElementById("live2dSet").checked = isLive2dEnabled();
}
